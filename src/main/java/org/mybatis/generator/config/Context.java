...
public class Context extends PropertyHolder {
    ...
    public void generateFiles(ProgressCallback callback,
        ...
        if (introspectedTables != null) {
            for (IntrospectedTable introspectedTable : introspectedTables) {
                ...
                IntrospectedTable tt = removePublicColumn(introspectedTable);
                generatedJavaFiles.addAll(tt
                        .getGeneratedJavaFiles());
                generatedJavaFiles.addAll(pluginAggregator
                        .contextGenerateAdditionalJavaFiles(tt));
            }
        }
    ...
    public IntrospectedTable removePublicColumn(IntrospectedTable table){
        IntrospectedTable introspectedTable = table;
        publicColumn = super.getProperties().getProperty("publicColumn");
        if(null == publicColumn){
            return table;
        }
        List<IntrospectedColumn> allColumns = introspectedTable.getBaseColumns();
        System.out.println(publicColumn);
        List<String> strings = Arrays.asList(publicColumn.split(","));
        List<IntrospectedColumn> introspectedColumns = removeBaseColumns(allColumns, strings);
        introspectedTable.setBaseColumns(introspectedColumns);
     ...
     * @return:
     **/
    public List<IntrospectedColumn> removeBaseColumns(List<IntrospectedColumn> allColumns,List<String> keys){
        List<String> realKey = new ArrayList<>();
        for(IntrospectedColumn column:allColumns){
            for(String key:keys){
                if(key.equals(column.getActualColumnName())){
                    realKey.add(key);
                    break;
                }
            }
        }
        int[] z = new int[realKey.size()];
        int i = 0;
        int j = 0;
        for(IntrospectedColumn column:allColumns){
            for(String key:keys){
        ...
        for(int x = z.length-1;x>=0;x--){
            IntrospectedColumn remove = allColumns.remove(z[x]);
        }
        return allColumns;
    }
