...
import com.wei.enums.SampleSliceTypeEnum;
...
public class DateUtil{
    ...
    public static String getDateByLong(Long l,String format){
    ...
    public static String getMonthByStringDate(String date,String split){
        String res = "";
        if(StringUtils.isNotBlank(split) && date.contains(split)){
            String[] sp = date.split(split);
            res = sp[0]+sp[1];
        }else{
            res = date.substring(0, 6);
        }
        return res;
    }
    ...
    /**
     * @Description 获取自然半年月份数据 (YYYYMM)
     * @Author: weijunjie
     * @Date: 2021/1/27 13:53
     **/
    public static List<String> getNaturalHalfYear(String month){
        String[] monthIndex = {"01","02","03","04","05","06","07","08","09","10","11","12"};
        ArrayList<String> strings = new ArrayList<>();
        String s = month.replaceAll("-", "");
        if(s.length() != 6){
            s = s.substring(0,6);
        }
        int theIndex = 0;
        ...
        int j = 0;
        if(theIndex>5)j=6;
        for(int i=j;i<monthIndex.length;i++){
            if(i<=theIndex){
                strings.add(year+monthIndex[i]);
            }
        }
    ...
    public static void main(String[] args) {
        String s = DateUtil.formatDate(new Date(), "YYYYMMddHHmmssSSS");
     ...
     **/
    public static String getLastMonth(String thisMonth,Integer i){
        String f = "yyyy-MM";
        SimpleDateFormat sf = new SimpleDateFormat(f);
        Date date = parseStrToDate(thisMonth, f);
        GregorianCalendar gregorianCalendar = new GregorianCalendar();
        gregorianCalendar.setTime(date);
        gregorianCalendar.add(Calendar.MONTH,0-i);
        return sf.format(gregorianCalendar.getTime());
    }

    /**
     * 获得传入日期的上N个月
     * @Title: getLastNMonth
     * @author: dy.yin 2021/1/19 10:34
     * @param: [thisMonth, i]
     * @return: java.lang.String
     */
    public static String getLastNMonth(String thisMonth,Integer i){
        String f = "yyyyMM";
        SimpleDateFormat sf = new SimpleDateFormat(f);
        Date date = parseStrToDate(thisMonth, f);
        GregorianCalendar gregorianCalendar = new GregorianCalendar();
     ...
     * 获得传入日期的下N个月
     * @Title: getNextNMonth
     * @author: dy.yin 2021/1/14 14:59
     * @param: [thisMonth  传入的时间格式202012, i下几个月  ]
     * @return: java.lang.String
     * @throws  例子:传入202012 ,4 得到的是202104
     */
    public static String getNextNMonth(String thisMonth,Integer i){
        String f = "yyyyMM";
        SimpleDateFormat sf = new SimpleDateFormat(f);
        Date date = parseStrToDate(thisMonth, f);
        GregorianCalendar gregorianCalendar = new GregorianCalendar();
        gregorianCalendar.setTime(date);
        gregorianCalendar.add(Calendar.MONTH,i);
        return sf.format(gregorianCalendar.getTime());
    }
     ...
     * @return yyyy-MM日期集合
     */
    public static List<String> getMonthListOfDate(String beginDateStr, String endDateStr, String split) {
        String format = "yyyy"+split+"MM";
        // 指定要解析的时间格式
        ...
        try {
            ...
            while (beginGC.getTime().compareTo(endGC.getTime()) <= 0) {
                sRet = beginGC.get(Calendar.YEAR) + split
                        + String.format("%02d", (beginGC.get(Calendar.MONTH) + 1));
                list.add(sRet);
                // 以月为单位，增加时间
    ...
    /**
     * @Description 获取月末日期字符串  YYYY-MM-DD
     * @Author: weijunjie
     * @Date: 2020/11/11 13:49
     **/
    public static String getLastDayYYYY_MM_DD(String MonthIndex){
        String[] split = MonthIndex.split("-");
        Date lastDayOfMonth = getLastDayOfMonth(Integer.parseInt(split[0]), Integer.parseInt(split[1]));
        return formatDate(lastDayOfMonth,DATE_FORMAT_YYYY_MM_DD);
    }

    /**
     * @Description 获取月初日期字符串  YYYY-MM-DD
     * @Author: weijunjie
     * @Date: 2020/11/11 13:49
     **/
    public static String getFirstDayYYYY_MM_DD(String MonthIndex){
        String[] split = MonthIndex.split("-");
        Date lastDayOfMonth = getFirstDayOfMonth(Integer.parseInt(split[0]), Integer.parseInt(split[1]));
        return formatDate(lastDayOfMonth,DATE_FORMAT_YYYY_MM_DD);
    }
     ...
     * @Description 时间拆分方法
     **/
    public static List<Map<String, String>> sliceSampleDateRange(String startDate, String endDate,
                                                                 SampleSliceTypeEnum sliceType, int interval) {
            ...
            switch (sliceType){
                case NATURAL_YEAR:
                    ...
                    while (startYear <= endYear) {
                        ...
                        map.put("from", startYear+"-01-01");
                        map.put("to", startYear+"-12-31");
                        list.add(map);
                        startYear++;
                    }
                    break;
                case NATURAL_HALF_YEAR:
                    sliceByHalfYear(startCal, endCal, sdf, list);
                    break;
                case NATURAL_QUARTER:
                    sliceByQuarter(startCal, endCal, sdf, list);
                    break;
                case MANUAL_MONTH:
                    sliceByInterval(startCal, endCal, sdf, list, interval);
                    break;
                default:
                    return null;
            }
    ...
    private static void sliceByHalfYear(Calendar startCal, Calendar endCal, SimpleDateFormat sdf, List<Map<String, String>> list){
        int startYear = startCal.get(Calendar.YEAR);
        int endYear = endCal.get(Calendar.YEAR);
        int startMonth = startCal.get(Calendar.MONTH);
        int endMonth = endCal.get(Calendar.MONTH);
        Calendar sGC = Calendar.getInstance();
        Calendar eGC = Calendar.getInstance();
        if(startMonth < 6){
            sGC.set(startYear, 0, 1, 0, 0, 0);
        }else{
            sGC.set(startYear, 6, 1, 0, 0, 0);
        }
        if(endMonth < 6){
            eGC.set(endYear, 5, 30, 0, 0, 0);
        }else{
            eGC.set(endYear, 11, 31, 0, 0, 0);
        }
        while (sGC.getTime().compareTo(eGC.getTime()) < 0) {
            String mapStart = sdf.format(sGC.getTime());
            sGC.add(Calendar.MONTH, 6);
            Calendar mapEndGC = (Calendar)sGC.clone();
            mapEndGC.add(Calendar.SECOND, -1);
            String mapEnd = sdf.format(mapEndGC.getTime());
            Map<String, String> map = new HashMap<>();
            map.put("from", mapStart);
            map.put("to", mapEnd);
            list.add(map);
        }
    }

    private static void sliceByQuarter(Calendar startCal, Calendar endCal, SimpleDateFormat sdf, List<Map<String, String>> list){
        int startYear = startCal.get(Calendar.YEAR);
        int endYear = endCal.get(Calendar.YEAR);
        int startMonth = startCal.get(Calendar.MONTH);
        int endMonth = endCal.get(Calendar.MONTH);
        ...
        if(startMonth < 3){
            ...
            sGC.set(startYear, 3, 1, 0, 0, 0);
        }else if(startMonth < 9){
            sGC.set(startYear, 6, 1, 0, 0, 0);
        }else{
            sGC.set(startYear, 9, 1, 0, 0, 0);
        }
        if(endMonth < 3){
            eGC.set(endYear, 2, 31, 0, 0, 0);
        }else if(endMonth < 6) {
            eGC.set(endYear, 5, 30, 0, 0, 0);
        }else if(endMonth < 9) {
            eGC.set(endYear, 8, 30, 0, 0, 0);
        }else{
            eGC.set(endYear, 11, 31, 0, 0, 0);
        }
        while (sGC.getTime().compareTo(eGC.getTime()) < 0) {
            String mapStart = sdf.format(sGC.getTime());
            sGC.add(Calendar.MONTH, 3);
            Calendar mapEndGC = (Calendar)sGC.clone();
            mapEndGC.add(Calendar.SECOND, -1);
            String mapEnd = sdf.format(mapEndGC.getTime());
            Map<String, String> map = new HashMap<>();
    ...
    private static void sliceByInterval(Calendar startCal, Calendar endCal, SimpleDateFormat sdf, List<Map<String, String>> list, int interval){
        if(interval < 1 || interval > 11){
            throw new RuntimeException("指定月数值范围无效.");
        }
        int startYear = startCal.get(Calendar.YEAR);
        int endYear = endCal.get(Calendar.YEAR);
        int startMonth = startCal.get(Calendar.MONTH);
        int endMonth = endCal.get(Calendar.MONTH);
        int endMaxDay = endCal.getActualMaximum(Calendar.DAY_OF_MONTH);
        Calendar sGC = Calendar.getInstance();
        Calendar eGC = Calendar.getInstance();
        sGC.set(startYear, startMonth, 1, 0, 0, 0);
        eGC.set(endYear, endMonth, endMaxDay, 0, 0, 0);
        while (sGC.getTime().compareTo(eGC.getTime()) < 0) {
            String mapStart = sdf.format(sGC.getTime());
            sGC.add(Calendar.MONTH, interval);
            Calendar mapEndGC = (Calendar)sGC.clone();
            mapEndGC.add(Calendar.SECOND, -1);
            String mapEnd = sdf.format(mapEndGC.getTime());
            Map<String, String> map = new HashMap<>();
